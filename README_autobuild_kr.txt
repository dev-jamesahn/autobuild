...
공통 설정은 ~/.config/autobuild_common.env에 둡니다. 이 파일에는 SMTP,
Samba 업로드 경로처럼 모든 빌드 항목이 공유하는 값을 정의합니다.
각 타겟별 env 파일은 공통 파일을 source한 뒤 repo, branch, build command,
artifact 경로처럼 타겟별 값만 정의합니다. 기존 openwrt_autobuild.env와
autobuild_samba_upload.env는 호환용 wrapper로 유지할 수 있습니다.
